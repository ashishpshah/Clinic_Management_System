...
namespace Clinic_Management_System.Areas.Admin.Controllers
{
	...
	public class AccessController : BaseController<ResponseModel<UserMenuAccess>>
	{
		...
		public ActionResult Index()
		{
			...
			var listRole = _context.Roles.AsNoTracking().ToList().Where(x => x.Id > 1).OrderBy(x => x.DisplayOrder).Select(x => new SelectListItem_Custom(x.Id.ToString(), x.Name, "R")).Distinct().ToList();
		...
		public ActionResult Partial_AddEditForm(long RoleId = 0, long UserId = 0)
		{
			...
			if (RoleId > 1 && UserId > 0)
			{
				...
				{
					if (_context.UserMenuAccesses.AsNoTracking().ToList().Any(x => x.UserId == UserId && x.RoleId == RoleId && x.MenuId == item.MenuId && x.RoleId > 1))
					{
						var access = _context.UserMenuAccesses.AsNoTracking().ToList().Where(x => x.UserId == UserId && x.RoleId == RoleId && x.MenuId == item.MenuId && x.RoleId > 1).FirstOrDefault();
		...
		public ActionResult Save(ResponseModel<UserMenuAccess> viewModel)
		{
			try
			{
				if (viewModel != null && viewModel.Obj != null)
				{
					...
					List<UserMenuAccess> listUserMenuAccess = _context.UserMenuAccesses.AsNoTracking().ToList().Where(x => x.UserId == viewModel.Obj.UserId && x.RoleId == viewModel.Obj.RoleId).ToList();
